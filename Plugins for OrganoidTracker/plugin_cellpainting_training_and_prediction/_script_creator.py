...
def create_training_script(window: Window):
    ...
    run_script_creator.create_run_script(output_folder, "train_cell_painting")
    config = ConfigFile("train_cell_painting", folder_name=output_folder)
    config.get_or_default("input_file", "training_and_validation_dataset" + FILES_LIST_EXTENSION)
    config.save()

    _show_success_confirmation(output_folder, "train_cell_painting")
...
def create_prediction_script(window: Window):
    ...
    with open(os.path.join(output_folder, "prediction_dataset" + FILES_LIST_EXTENSION), "w") as handle:
        json.dump(files_json, handle)
    run_script_creator.create_run_script(output_folder, "predict_cell_painting")
    config = ConfigFile("predict_cell_painting", folder_name=output_folder)
    ...
    _show_success_confirmation(output_folder, "predict_cell_painting")
