...
def build_model(config: NetworkConfig) -> tf.keras.Model:
    ...
    layer, to_concat_layer = _conv_block(n, layer, filters=filter_sizes[1], kernel=(1, 3, 3), pool_size=(1, 2, 2),
                                         pool_strides=(1, 2, 2), make_2d=config.patch_size_z <= 1, name="down1")
    to_concat.append(to_concat_layer)
    layer, to_concat_layer = _conv_block(n, layer, filters=filter_sizes[2], make_2d=config.patch_size_z <= 4, name="down2")
    to_concat.append(to_concat_layer)
    layer, to_concat_layer = _conv_block(n, layer, filters=filter_sizes[3], make_2d=config.patch_size_z <= 2, name="down3")
    to_concat.append(to_concat_layer)
    layer, to_concat_layer = _conv_block(n, layer, filters=filter_sizes[4], make_2d=config.patch_size_z <= 1, name="down4")
    to_concat.append(to_concat_layer)

    layer = _deconv_block(n, layer, to_concat.pop(), filters=filter_sizes[4], make_2d=config.patch_size_z <= 1, name="up1")
    layer = _deconv_block(n, layer, to_concat.pop(), filters=filter_sizes[3], make_2d=config.patch_size_z <= 2, name="up2")
    layer = _deconv_block(n, layer, to_concat.pop(), filters=filter_sizes[2], make_2d=config.patch_size_z <= 4, name="up3")
    layer = _deconv_block(2, layer, to_concat.pop(), filters=filter_sizes[1], make_2d=config.patch_size_z <= 1, kernel=(1, 3, 3), strides=(1, 2, 2), dropout=False, name="up4")
...
def _conv_block(n_conv, layer, filters, kernel=(3, 3, 3), pool_size=(2, 2, 2), pool_strides=(2, 2, 2),
                make_2d: bool = False, dropout: bool = False, name: Optional[str] = None):
    if make_2d:  # Used to change all the z values to 1
        kernel = (1, kernel[1], kernel[2])
        pool_size = (1, pool_size[1], pool_size[2])
        pool_strides = (1, pool_strides[1], pool_strides[2])
...
def _deconv_block(n_conv, layer, to_concat, filters, kernel=(3, 3, 3), strides=(2, 2, 2),
                  dropout: bool = False, make_2d: bool = False, name: Optional[str] = None):
    if make_2d:  # Used to change all the z values to 1
        kernel = (1, kernel[1], kernel[2])
